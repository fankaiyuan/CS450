...
const char *WINDOWTITLE = { "Project 3 -- Kaiyuan Fan" };
const char *GLUITITLE   = { "User Interface Window" };

const char * Texture0 = "worldtex.bmp";
const char * Texture1 = "watermelon.bmp";
...
int Sec_width, Sec_height;// texture pirture wirdth and height
GLuint tex0;
GLuint tex1;//
...
// 
void init_texture() {
	int tex0_width, tex1_width;
	int tex0_height, tex1_height;
	...
	unsigned char * Texture1 = BmpToTexture("wood.bmp", &tex1_width, &tex1_height);
	...
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, tex0_width, tex0_height, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture1);
...
Display( )
{
	...
	// mapping the texture
	if (Tex_type == No_texture)
	{
		glDisable(GL_TEXTURE_2D); // disable the texture

	}
	else if (Tex_type == With_texture)
	{
		Distort = FALSE;
	...
	else if (Tex_type == Distort_texture) {
		Distort = TRUE;
	}
	else 
	{
...
InitMenus( )
{
	...
	glutAddMenuEntry("With Texture", With_texture);
	glutAddMenuEntry("Distort Texture", Distort_texture);
	glutAddMenuEntry("Second Texture", Sec_texture);
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				p->s = (lng*Time + M_PI) / (2.*M_PI); // Distortion
				p->t = (lat + M_PI) / (2.*M_PI);
			}
			else
			{
