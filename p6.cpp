...
const char *WINDOWTITLE = { "Project 6 -- Kaiyuan Fan" };
...
Display( )
{
	...
	for (int j = 0; j < curves; j++) {
		float angle = 2*PI * j / curves;
		struct Curve curve;
		struct Point p0 = { 0.5 * cos(angle - PI / 6), 0.5 * sin(angle - PI / 6) , 0. };
		curve.p0 = p0;
		struct Point p1 = { 1 * cos(angle- PI / 6), 1* sin(angle- PI / 6) , -Time };
		curve.p1 = p1;
		struct Point p2 = { 1 * cos(angle), 1*sin (angle), -Time  };
		curve.p2 = p2;
		struct Point p3 = { 0.5 * cos(angle), 0.5 * sin(angle), 0.};
		curve.p3 = p3;
	...
	for (int j = 0; j < curves; j++) {
		...
		curve.p0 = p0;
		struct Point p1 = { 2 * cos(angle - PI / 6), 2 * sin(angle - PI / 6) , Time };
		curve.p1 = p1;
		struct Point p2 = { 2 * cos(angle),	2 * sin(angle), Time };
		curve.p2 = p2;
	...
	for (int j = 0; j < num_curves; j++) {
		float angle = 2 * PI * j / num_curves;
		struct Curve curve;
		struct Point p0 = { 0., 0., 0. };
		curve.p0 = p0;
		struct Point p1 = { 1.5 * cos(angle - PI / 6), 1.5 * sin(angle - PI / 6) , 0. };
		curve.p1 = p1;
		struct Point p2 = { 1.5 * cos(angle),	1.5 * sin(angle), 0. };
		curve.p2 = p2;
		struct Point p3 = { 0., 0., 0. };
		curve.p3 = p3;
		//RotateZ(&p0, 15., 0., 0., 0.);
	...
	glDisable( GL_DEPTH_TEST );
	glColor3f( 0., 1., 1. );
	DoRasterString( 0., 1., 0., "Geometric Modeling" );
